...
{prefix}/_predictions/v1/{id}.bin             encrypted input predictions
...
These rules keep the store consistent under expiry:
...
- Expire artifacts by age since they were written. Artifact objects are immutable, so a hot entry expires too and is uploaded again on the next miss. Pick an age longer than the interval at which your builds change.
- Expire `_predictions/` independently. Losing a prediction runs the dep-info pre-pass; it does not invalidate an artifact.

An expired artifact is a miss. The daemon's remote key index can report it present for up to `cache.remote_key_cache_refresh_secs` after expiry; the fetch then misses and the crate compiles as usual. An expired prediction only brings back the dep-info pre-pass.
...
```json title="lifecycle.json"
{
  "Rules": [
    ...
    { "ID": "kache-predictions", "Status": "Enabled", "Filter": { "Prefix": "artifacts/_predictions/" }, "Expiration": { "Days": 30 } },
...
mc ilm rule add --prefix "artifacts/_predictions/" --expire-days 30 myminio/my-kache
...
[ ! -d "$root/_predictions" ] || find "$root/_predictions" -type f -mtime +30 -delete
