...
| `KACHE_REMOTE_PREDICTION_KEY` | none | unset | With `input_predictions` enabled, share rows through the configured remote. Set the same 64-character hex key on each trusted builder. Kache encrypts source paths and uses keyed object names; without the key, predictions stay local. Rows with arbitrary environment values stay local. Keep this key out of untrusted CI jobs. |
...
Generate a remote prediction key with `openssl rand -hex 32`. Store it as a secret and give the same value only to builders that should share source-closure metadata.
