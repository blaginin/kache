...
If you need a compiler Kache does not cache yet, such as CUDA or HIP, [use sccache as a fallback](#use-sccache-as-a-fallback). Kache caches what it supports and passes the rest to sccache. You do not have to choose one or the other.
...
Kache still caches everything it supports. Compiler invocations it passes through go to sccache instead of the bare compiler. Keep the fallback as long as it helps; `KACHE_FALLBACK` sets the same option for a single build.
